...
export async function POST(req: Request) {
  try {
    ...
    // Vérifie si l'utilisateur existe (sans révéler à l’extérieur)
    const user = await prisma.user.findUnique({
      where: { email },
      select: { id: true }, // ❌ on ne demande plus "status"
    });
    ...
    const [recent, countToday] = await Promise.all([
      prisma.verificationToken.findFirst({
        where: {
          ...
          // ton schéma est en snake_case: created_at
          created_at: { gt: since } as any,
        },
        select: { id: true },
      }),
    ...
    verifyUrl.searchParams.set("token", token); // ✅ on n’envoie pas l’email
