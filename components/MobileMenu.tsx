...
export default function MobileMenu({ open, onClose }: Props) {
  // si fermé, ne rend rien (zéro overhead et évite les focus fantômes)
  if (!open) return null;

  const panelRef = useRef<HTMLDivElement>(null);

  // lock scroll à l’ouverture
  useEffect(() => {
  ...
  return (
    <Portal>
      ...
      {/* panneau plein-écran au-dessus de tout */}
      <div
        ref={panelRef}
        role="dialog"
        aria-modal="true"
        aria-label="Main menu"
        className="fixed inset-0 z-[1000] flex flex-col bg-slate-950/95 transition-transform duration-200"
      >
        {/* barre supérieure */}
        <div className="flex items-center justify-between px-4 py-4 border-b border-white/10">
          ...
          <button
            ...
            className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-emerald-400/60"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
