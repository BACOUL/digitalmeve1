// components/Hero.tsx — v2 "use client";

import Link from "next/link"; import { useCallback, useRef, useState } from "react"; import { ShieldCheck, Radar, Sparkles, ArrowRight } from "lucide-react";

export default function Hero() { const fileInputRef = useRef<HTMLInputElement | null>(null); const [isDragging, setDragging] = useState(false); const [busy, setBusy] = useState(false); const [status, setStatus] = useState<string | null>(null);

// TODO: Replace with your real in‑browser protect/verify logic const handleFile = useCallback(async (file: File) => { try { setBusy(true); setStatus("Verifying in your browser…"); await new Promise((r) => setTimeout(r, 1200)); setStatus("Verified ✔ — tamper‑proof certificate ready."); } catch (e) { setStatus("Something went wrong. Try another file."); } finally { setBusy(false); } }, []);

const onDrop = useCallback( async (e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); e.stopPropagation(); setDragging(false); const f = e.dataTransfer.files?.[0]; if (f) await handleFile(f); }, [handleFile] );

const onChange = useCallback( async (e: React.ChangeEvent<HTMLInputElement>) => { const f = e.target.files?.[0]; if (f) await handleFile(f); e.currentTarget.value = ""; // allow same file re‑selection }, [handleFile] );

return ( <section
id="hero"
aria-label="DigitalMeve — Invisible proof. Visible trust."
className="relative overflow-hidden min-h-[100svh] pb-24"
> {/* Background FX (kept) */} <div aria-hidden className="pointer-events-none absolute inset-0 -z-10"> <div className="aurora aurora-1 max-sm:opacity-70 max-sm:[filter:blur(6px)]" /> <div className="aurora aurora-2 max-sm:opacity-70 max-sm:[filter:blur(6px)]" /> <div className="aurora aurora-3 max-sm:opacity-70 max-sm:[filter:blur(6px)]" /> <div className="beam" /> <div className="noise" /> <div className="absolute inset-x-0 top-0 h-12 bg-white/90 [mask-image:linear-gradient(to_bottom,black,transparent)]" /> </div>
...
{/* Content */}
  <div className="relative mx-auto max-w-6xl px-4 sm:px-5 pt-20 sm:pt-24 pb-5 sm:pb-20 text-center">
    ...
    >
      ...
      {/* Sample file action (simulate or wire a real sample) */}
      <button
        type="button"
        onClick={async () => {
          setBusy(true);
